namespace rocket_bot
{
    public class Rocket
    {
        ...
        public Vector GetNextRocketCheckpoint(Level level) =>
            level.Checkpoints[TakenCheckpointsCount % level.Checkpoints.Length];

        public bool IsCompleted(Level level) => Time >= level.MaxTicksCount;
        ...
        public Rocket IncreaseCheckpoints() =>
            new Rocket(Location, Velocity, Direction, Time, TakenCheckpointsCount + 1);

        protected bool Equals(Rocket rocket) =>
            Location.Equals(rocket.Location) && Velocity.Equals(rocket.Velocity) &&
